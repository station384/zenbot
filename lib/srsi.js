...
module.exports = function srsi(s, key, rsi_periods, k_periods, d_periods) {
  let samplesRequiredForStochRSI = rsi_periods + k_periods +d_periods + 1
  ...
  if (s.lookback.length >= samplesRequiredForStochRSI - 1) {
    ...
    if(RSI.length >= samplesRequiredForStochRSI) {
      ...
      s.period[key + '_K'] = percentK[0] == 0 ? 0 : mathjs.round(percentK[0]+0.0, 2)
      s.period[key + '_D'] = percentD[0] == 0 ? 0 : mathjs.round(percentD[0]+0.0, 2)
